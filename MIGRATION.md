...
This release (ADR-0013, wave 1 exit surfaces) adds six author-visible surfaces. Every one of them is additive: a book already in progress needs no action to keep working exactly as it did before, and every new default is chosen so that omitting the new capability entirely reproduces the old behavior.
