...
**Turns Claude into a governed non-fiction book studio: specialist subagents draft, a plain-Markdown project bible holds the truth, and a deterministic quality gate decides what counts as done. For Claude Code.**
...
2. **Run `/nonfiction-studio:quick-scan`** and paste 500 to 1000 words of your own prose. You get back a voice profile measured by the same deterministic engine the plugin uses everywhere else, a claim scan flagging sentences that assert a fact needing a source, and a one-paragraph read on what it is about. The measured parts and the model-judged parts are kept visibly distinct.
3. **Run `/nonfiction-studio:tour`** to watch the quality gate work: it passes on the bundled sample book, blocks on a planted, realistic defect with a named reason, then passes again once the defect is fixed. The tour runs from a disposable copy; nothing touches the shipped example.
4. **Ready to write your own book?** Run `/studio` (or `/nonfiction-studio:studio` if another installed plugin also defines `/studio`) and choose "Start a new book."
...
- **Just want to see if it is any good** - `/nonfiction-studio:quick-scan` with your own prose, then `/nonfiction-studio:tour`. Five minutes, no setup.
...
- **Ready to start a book** - `/studio`, then "Start a new book." Block out 45 to 90 minutes for intake.
- **Something looks wrong in your project** - `/nonfiction-studio:doctor` first, always.
