...
Choose "Start a new book," or run `/nonfiction-studio:nfs-new-book` directly. That scaffolds the project bible (the book's canonical reference files under `context/`, `structure/`, `chapters/`, `research/`, and `production/`) and hands off to `/nonfiction-studio:nfs-interview`, which is a real 45 to 90 minute session that captures your book's thesis, audience, and voice. That duration is intentional and stays honest; nothing above shortens it or is a substitute for it. Quick-scan and the tour exist so you can see what the studio actually does before spending that time.
...
- [docs/reference/skills/nfs-quick-scan.md](reference/skills/nfs-quick-scan.md) and [docs/reference/skills/nfs-tour.md](reference/skills/nfs-tour.md) - the full contract for each skill used above
- [docs/reference/skills/nfs-start.md](reference/skills/nfs-start.md) - the guided front door that routes to everything else
