...
2. **Cluster rule.** A tight cluster of consecutive sentences all supported by the same evidence ledger entry carries one `[claim: EV-NNNN]` marker at the end of the cluster, not one marker per sentence.
...
5. **No marker for non-factual sentences.** Opinion, transition, and narrative sentences carry no marker. Coverage is measured over factual sentences only. `fact-checker` determines which sentences are factual.
...
Every `[UNVERIFIED]` tag is also counted as one open claim.
...
- `bin/ns-doctor` (TSK-028 (ns-doctor engine)): validates that every `[claim: EV-NNNN]` references an existing EV entry; reports broken markers.
