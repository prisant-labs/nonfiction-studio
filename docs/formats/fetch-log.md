...
**`scanPromptInjection` is deliberately tuned for high precision over high recall.** A version that flagged on a single matched pattern alone produced false positives on routine web content sharing the same vocabulary (status pages, documentation corrections, ordinary technical writing); see the scanner's own doc comment in `hooks/lib/scrub-engine.mjs` for the corroboration rule this led to. Consequence for a reader of this log: `flagged: true` is a comparatively rare, higher-confidence signal worth a human's attention, while `flagged: false` is common even for legitimate assistant-directed phrasing the scanner chose not to promote without a second corroborating signal. Treat a low `flagged: true` rate as the design working, not as evidence the scanner stopped running.
...
- `scanPromptInjection(text)` detects instruction-override phrasing directed at an assistant (finding types `injection.prompt-override.countermand`, `injection.prompt-override.role-redefinition`, `injection.prompt-override.extraction`): attempts to countermand prior instructions, redefine the assistant's role or mode, or extract configuration or secrets. Each is a corroborated finding, never a single matched pattern alone: a category match is promoted only when a second, distinct category also matches on the same line, or when an explicitly assistant-directed referent ("your instructions", "you are now", "the system prompt") is also present on that line. A fourth category (a line beginning `System:`/`Assistant:`/`Admin:`, meant to catch a forged conversation-role message) was dropped: it had no semantic check beyond the line-initial label format, which is how transcripts, status pages, and quoted dialogue are routinely written, and produced false positives on all three tested.
