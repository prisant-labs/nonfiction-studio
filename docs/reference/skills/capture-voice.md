---
title: "capture-voice skill reference"
...
```
/nonfiction-studio:capture-voice
```

No argument is required. The skill walks the author through sample submission.

Alternate entry points:
- Via `intake-interview` Step 6: that skill suggests `capture-voice` when `context/style-profile.md` is absent after the brief is confirmed
- Via the `studio` dispatcher: routes here when the author says "capture my voice", "set my writing style", or "I have samples to share"
...
6. **Close with suggestions.** Offers `outline-book` or `intake-interview` as optional next steps based on project state.
