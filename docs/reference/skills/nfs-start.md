...
See [nfs-start.example.md](./nfs-start.example.md) for a condensed transcript of a `studio` Path 2 session over the committed two-chapter sample book "The Quiet Network" (see `examples/sample-book/`). The example is grounded in the committed progress.json, meta.json, and chapter-list.md fixtures.
