...
`ns-claims` is one of the nine shipped CLIs per D-05 (five shipped CLIs via bin/). In
...
- [ns-overlap CLI reference](./ns-overlap.md) - reuses this module's quote-anchor span helper for its quoted-span exclusion rule
- [nfs-fact-check skill reference](../skills/nfs-fact-check.md) - skill that invokes the fact-checker agent
