...
**Consequence for TSK-071 (memory wiring):** D-09 (learning checker agents) proceeds with `memory: project` in `fact-checker`, `voice-guardian`, and `continuity-checker` agent frontmatter. No fallback cache mechanism is needed. TSK-071 (memory wiring) can wire all three agents to `memory: project`. Planning documents for D-09 (learning checker agents) that reference project-scope memory paths as `.claude/agent-memory/<agent-name>/` must be corrected to the plugin-prefixed form `.claude/agent-memory/nonfiction-studio-<agent-name>/` discovered in this spike (this covers the reset-path references in AR-04 (agent team and orchestration) and AR-07 (security, privacy, and safety)).

**Directory naming note for TSK-071 (memory wiring):** The actual on-disk directory is `.claude/agent-memory/nonfiction-studio-spike-memory/`, not `.claude/agent-memory/spike-memory/`. Any `.gitignore` or `.studio/` scaffolding that references memory paths must use the plugin-prefixed form `nonfiction-studio-<agent-name>`, not the bare agent name. Update the A-02 PF-04 path example accordingly when authoring TSK-071 (memory wiring). The committed stub agent's body text referenced the bare-slug path `.claude/agent-memory/spike-memory/` yet the agent accessed the plugin-prefixed directory, which is mechanistic evidence that the `memory: project` frontmatter field, not any body-text path literal, controls which directory the platform creates and provides to the agent. TSK-071 (memory wiring) should therefore treat body-text path mentions as documentation only and never as configuration.
