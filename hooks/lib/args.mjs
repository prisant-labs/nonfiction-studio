// what-it-is:   the uniform CLI flag parser for eight of the nine engine CLIs
...
// why:          the S-07 (hooks and scripts) flag families are shared across CLIs; one parser keeps the contract uniform
//               and ensures every CLI that imports it exits 2 on bad arguments with the same message shape
// used-by:      imported by bin/ns-gate, bin/ns-claims, bin/ns-stylometry, bin/ns-scrub, bin/ns-doctor, bin/ns-notes, bin/ns-status,
//               bin/ns-overlap (every engine CLI except ns-statusline, which parses its own narrower flag set directly)
