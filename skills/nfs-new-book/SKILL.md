...
**Where this step returns.** Step 7 is reachable two ways: from Step 6 below, on a fresh NEWINIT run, and from Step 1a above, on a REINIT run (both the SCAN_DONE sub-branch and the MISSING-files sub-branch). Every branch below ends with "Continue to Step 8" - that instruction applies ONLY when this run reached Step 7 from Step 6. When Step 1a invoked Step 7 (any REINIT entry), stop here instead, whichever branch below concluded: Step 1a's own warning has already reported this invocation's outcome, and Step 8's "Book project has been initialized" report must never print for a REINIT run, since no fresh initialization happened.
